...
# -------------------------------------------------------------
# PHASE 1: TRAIN BASELINE ERM & GENERATE OVERFIT/UNDERFIT GRAPH
# -------------------------------------------------------------
def run_baseline_erm(device, train_loader, val_loader, num_classes=75, epochs=5):
    print("\n" + "="*70, flush=True)
    print(">>> PHASE 1: Training Standard ERM Baseline (Demonstrating Shortcut Failure)", flush=True)
    print("="*70, flush=True)
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"Epoch [{epoch:02d}/{epochs:02d}] "
              ...
              f"Val Worst Group: {worst_acc*100:5.1f}% | Collapsed: {collapsed}/300", flush=True)
    ...
    df_hist.to_csv("plots/experiments_75/baseline_history.csv", index=False)

    # Plot Overfit/Underfit Analysis (300 groups)
    plot_baseline_curves(history, best_group_accs, "plots/experiments_75/baseline_erm_curves.png")
...
def plot_baseline_curves(history, best_group_accs, out_path):
    ...
    ax1.set_title("75 Species: Loss Trajectory & Overfitting Identification", fontsize=13, fontweight='bold')
    ...
    ax2.set_title("Accuracy Disparity (75 Species, 300 Groups)", fontsize=13, fontweight='bold')
    ...
    # 3. Distribution across 300 groups
    ax3 = axes[1, 0]
    ...
    plt.suptitle("75-Species Butterfly Classifier: Baseline Overfit/Underfit & Shortcut Collapse", fontsize=16, fontweight='bold', y=0.98)
    plt.savefig(out_path, bbox_inches='tight')
    plt.close()
    print(f"Baseline curves saved to: {out_path}", flush=True)

# -------------------------------------------------------------
# PHASE 2: BENCHMARK MITIGATION METHODS
# -------------------------------------------------------------
def run_dfr(base_net, val_loader, device, num_classes=75):
    print("\n" + "="*70, flush=True)
    print(">>> PHASE 2.1: Method 1 - Deep Feature Reweighting (DFR)", flush=True)
    print("="*70, flush=True)
    start_time = time.time()
    ...
    print(f"DFR Complete! Overall: {overall_acc*100:.1f}% | Worst Group: {worst_acc*100:.1f}% | Collapsed: {collapsed}/300 | Time: {duration:.2f}s", flush=True)

    torch.save(dfr_model.state_dict(), "plots/experiments_75/model_dfr.pt")
    plot_single_method("Method 1: Deep Feature Reweighting (DFR)", group_accs, worst_acc, overall_acc, collapsed, "plots/experiments_75/method1_dfr.png")
...
def run_group_dro(base_net, val_loader, device, num_classes=75):
    print("\n" + "="*70, flush=True)
    print(">>> PHASE 2.2: Method 2 - Group DRO (Minimax Optimization)", flush=True)
    print("="*70, flush=True)
    start_time = time.time()
    ...
    print(f"Group DRO Complete! Overall: {overall_acc*100:.1f}% | Worst Group: {worst_acc*100:.1f}% | Collapsed: {collapsed}/300 | Time: {duration:.2f}s", flush=True)

    torch.save(dro_model.state_dict(), "plots/experiments_75/model_group_dro.pt")
    plot_single_method("Method 2: Group DRO (Minimax)", group_accs, worst_acc, overall_acc, collapsed, "plots/experiments_75/method2_group_dro.png")
...
def run_shortcut_aug(device, val_loader, num_classes=75):
    print("\n" + "="*70, flush=True)
    print(">>> PHASE 2.3: Method 3 - Shortcut-Destructive Augmentation", flush=True)
    print("="*70, flush=True)
    start_time = time.time()
    ...
    print(f"Shortcut Aug Complete! Overall: {overall_acc*100:.1f}% | Worst Group: {worst_acc*100:.1f}% | Collapsed: {collapsed}/300 | Time: {duration:.2f}s", flush=True)

    torch.save(net.state_dict(), "plots/experiments_75/model_shortcut_aug.pt")
    plot_single_method("Method 3: Shortcut Augmentation (Grayscale/Erasing)", group_accs, worst_acc, overall_acc, collapsed, "plots/experiments_75/method3_shortcut_aug.png")
...
def run_jtt(base_net, val_loader, device, num_classes=75):
    print("\n" + "="*70, flush=True)
    print(">>> PHASE 2.4: Method 4 - Just Train Twice (JTT)", flush=True)
    print("="*70, flush=True)
    start_time = time.time()
    ...
    print(f"JTT Complete! Overall: {overall_acc*100:.1f}% | Worst Group: {worst_acc*100:.1f}% | Collapsed: {collapsed}/300 | Time: {duration:.2f}s", flush=True)

    torch.save(jtt_model.state_dict(), "plots/experiments_75/model_jtt.pt")
    plot_single_method("Method 4: Just Train Twice (JTT)", group_accs, worst_acc, overall_acc, collapsed, "plots/experiments_75/method4_jtt.png")
...
def plot_single_method(title, group_accs, worst_acc, overall_acc, collapsed, out_path):
    ...
    ax1.set_title(f"Accuracy Distribution (Collapsed Groups: {collapsed}/300)", fontsize=11, fontweight='bold')
    ...
    plt.suptitle(f"{title} (75 Species) | Overall: {overall_acc*100:.1f}% | Worst Group: {worst_acc*100:.1f}%", fontsize=13, fontweight='bold')
    plt.savefig(out_path, bbox_inches='tight')
    plt.close()

def plot_grand_comparison_75(results, out_path="plots/experiments_75/grand_benchmark_75_species.png"):
    ...
    ax3.set_title("Collapsed Groups out of 300 (Lower is Better)", fontsize=12, fontweight='bold')
    ...
    plt.suptitle("Grand Benchmark Report: 75 Species, 300 Subpopulation Groups", fontsize=15, fontweight='bold', y=1.02)
    plt.savefig(out_path, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    print(f"Executing 75-Species Benchmark on: {device}", flush=True)

    os.makedirs("plots/experiments_75", exist_ok=True)
    ...
    # 1. Baseline ERM
    base_net, erm_res = run_baseline_erm(device, train_loader, val_loader, num_classes=num_classes, epochs=5)
    results = [erm_res]

    # 2. DFR
    dfr_net, dfr_res = run_dfr(base_net, val_loader, device, num_classes=num_classes)
    results.append(dfr_res)

    # 3. Group DRO
    dro_res = run_group_dro(base_net, val_loader, device, num_classes=num_classes)
    results.append(dro_res)

    # 4. Shortcut Augmentation
    aug_res = run_shortcut_aug(device, val_loader, num_classes=num_classes)
    results.append(aug_res)

    # 5. JTT
    jtt_res = run_jtt(base_net, val_loader, device, num_classes=num_classes)
    results.append(jtt_res)

    # Summary
    df_results = pd.DataFrame(results)
    df_results.to_csv("plots/experiments_75/benchmark_summary.csv", index=False)
    print("\n" + "="*70, flush=True)
    print("75-SPECIES BENCHMARK SUMMARY TABLE:", flush=True)
    print(df_results.to_string(index=False), flush=True)
    print("="*70, flush=True)

    plot_grand_comparison_75(results)

    # Save champion model (DFR) to outputs/model.pt for test.py
